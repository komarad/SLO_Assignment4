...
int backward;
int pastSecond=second()-backward;
int pastMinute=second()-backward;
int pastHour=second()-backward;
...
int y = year(); 
  ...
  rotate(radians(6*(pastMinute+pastSecond/60.0)-90));
  minuteHand();
  popMatrix(); // end minuteHand context

  pushMatrix(); // start hourHand context
  rotate(radians(30*(pastHour+pastMinute/60.0)-90));
  hourHand();
